...
RateLimiter is a Redis backed object used to define one or more rules to rate
limit requests.
...
class RateLimiter(object):
    ...
    These rules are checked on .acquire() and we either return True or False
    based on if we can make the request,
    ...
    def add_condition(self, *conditions):
        ...
        Conditions can be given as:
            ...
            add_condition({'requests': 1, 'seconds': 10},
                          {'requests': 200, 'hours': 6})

            dict can contain 'seconds', 'minutes', 'hours', and 'days' time
            period parameters
        """
        ...
        for condition in conditions:
            ...
            if seconds > 0:
                if requests == 0:
                    self.log.warn('added block all condition (%s/%s)',
                                  requests, seconds)
                else:
                    self.log.debug(
    ...
    def block(self, key, seconds=0, minutes=0, hours=0, days=0):
        ...
        if not seconds:
            ...
            if not seconds:
                self.log.warn('''block called but no default block time.
                              not blocking''')
                return 0
    ...
    def acquire(self, key, block=True):
        """
        Tests whether we can make a request, or if we are currently being
        limited key - key to track what to rate limit
    ...
    def _make_ping(self, key):
        ...
        if min_requests == 0:
            self.log.warn('(%s) hit block all limit (%s/%s)', key,
                          min_requests, min_request_seconds)
        ...
        with self.redis.lock(lock_key, timeout=10):
            ...
            if blocked is not None:
                # block_ttl is None for last second of a keys life.
                # set min of 0.5
                if block_ttl is None:
                    block_ttl = 0.5
                self.log.warn('(%s) hit manual block. %ss remaining', key,
                              block_ttl)
                return False, block_ttl

            timestamp = time.time()

            for boundry_timestamp, (requests, seconds) in\
                    izip(boundry_timestamps, self.conditions):
                # if we dont yet have n number of requests
                # boundry_timestamp will be None and this condition wont be
                # limiting
                if boundry_timestamp is not None:
                    ...
                    if boundry_timestamp + seconds > timestamp:
                        self.log.warn(
                            ...
                            key, requests, seconds, boundry_timestamp +
                            seconds - timestamp)
            ...
            with self.redis.pipeline() as pipe:
                ...
                # if we never use this key again, let it fall out of the
                # DB after max seconds has past
...
if __name__ == '__main__':
    ...
    rate.add_condition({'requests': 40, 'minutes': 15},
                       {'requests': 400, 'days': 1})
